...
export const ChainedModal = ({ update, onEnd }) => {
  ...
  const handleSelect = (item) => {
    setCurrentPick(item);
    if (item.nextID) {
      QuestionList[item.nextID].replies.every((item) => !item['option']) &&
        setShowFalse();
...
export const NoReplyModal = () => {
  ...
  const [questionID, setQuestionID] = useState(
    randomPickFromArray(AllNoReplyQuestionIDs)
  );
  useEffect(() => {
  ...
  return (
    <View>
      <Questions
        questionInfo={QuestionList[questionID]}
        show={show}
      ></Questions>
    </View>
  );
...
export const Questions = ({
  ...
  onSelected,
  show,
  currentPick = { nextID: '1', reply: [], option: '' },
}) => {
  ...
  return (
    <ConversationModal modalShow={show}>
      {!currentPick.nextID ? (
        <Text style={{ fontFamily: 'pml', fontSize: 34 }}>
          {randomPickFromArray(currentPick.reply)}
        </Text>
      ) : (
        <>
